...
func (rule *RoundDollarAmountRule) evaluate(receipt *Receipt) int {
	points := 0
	if receipt.Total.int%100 == 0 {
		points = 50
	}
	return points
}
...
func (rule *PointTwoFiveRule) evaluate(receipt *Receipt) int {
	points := 0
	if receipt.Total.int%25 == 0 {
		points = 25
	}
	return points
}
...
func (rule *ItemsLengthRule) evaluate(receipt *Receipt) int {
...
func (rule *OddDayRule) evaluate(receipt *Receipt) int {
	points := 0
...
func (rule *TwoPMTo4PMRule) evaluate(receipt *Receipt) int {
	points := 0
